...
import { User, Profile } from '../../common/interfaces';
...
export class Signin {
  ...
  public async signIn() {
    const user: any = this.platform.is('cordova')
      ? await this.nativeGoogleLogin()
      : await this.webGoogleLogin();

    await this.sharedService.initUser(user)
  ...
  async webGoogleLogin(): Promise<User> {
    ...
    return { profile, isNewUser: userCredential.additionalUserInfo.isNewUser };
  };
}
